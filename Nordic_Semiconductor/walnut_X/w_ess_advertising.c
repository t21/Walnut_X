...
//#define ADV_LOG(...)
//#define ADV_LOG_PRINTF(...)
#define ADV_LOG NRF_LOG
#define ADV_LOG_PRINTF NRF_LOG_PRINTF
...
static ess_adv_evt_t                   m_adv_evt;          /**< Advertising event propogated to the main application. The event is either a transaction to a new advertising mode, or a request for whitelist or peer address.. */
static ess_advertising_evt_handler_t   m_evt_handler;      /**< Handler for the advertising events. Can be initialized as NULL if no handling is implemented on in the main application. */
static ess_advertising_error_handler_t m_error_handler;    /**< Handler for the advertising error events. */

static ess_adv_mode_t                  m_adv_mode_current; /**< Variable to keep track of the current advertising mode. */
static ess_adv_modes_config_t          m_adv_modes_config; /**< Struct to keep track of disabled and enabled advertising modes, as well as time-outs and intervals.*/
...
//        m_whitelist_reply_expected = true;
        m_evt_handler(ESS_ADV_EVT_WHITELIST_REQUEST);
    }
    else
    {
//        m_whitelist_reply_expected = false;
    ...
    switch (m_adv_mode_current) {
        case ESS_ADV_MODE_1:
            ADV_LOG("[ADV]: AdvMode=1");
            if (whitelist_has_entries(&m_whitelist)) {
                ADV_LOG(" +whitelist.\r\n");
                adv_params.timeout     = m_adv_modes_config.ess_adv_bonded_wl_on_timeout;
                adv_params.interval    = m_adv_modes_config.ess_adv_bonded_wl_on_interval;
                ...
                ADV_LOG(" -whitelist.\r\n");
                adv_params.timeout     = m_adv_modes_config.ess_adv_unbonded_fast_timeout;
                adv_params.interval    = m_adv_modes_config.ess_adv_unbonded_fast_interval;
                adv_params.fp          = BLE_GAP_ADV_FP_ANY;
                adv_params.p_whitelist = NULL;
            ...
            m_adv_evt = ESS_ADV_EVT_MODE_1;
            break;
            
        case ESS_ADV_MODE_2:
            ADV_LOG("[ADV]: AdvMode=2.\r\n");
            if (whitelist_has_entries(&m_whitelist)) {
                ADV_LOG(" +whitelist.\r\n");
                adv_params.timeout     = m_adv_modes_config.ess_adv_bonded_wl_off_fast_timeout;
                adv_params.interval    = m_adv_modes_config.ess_adv_bonded_wl_off_fast_interval;
                adv_params.fp          = BLE_GAP_ADV_FP_ANY;
                adv_params.p_whitelist = NULL;
//                m_advdata.flags        = BLE_GAP_ADV_FLAGS_LE_ONLY_GENERAL_DISC_MODE;
                err_code               = ble_advdata_set(&m_advdata, NULL);
                ...
                ADV_LOG(" -whitelist.\r\n");
                adv_params.timeout     = m_adv_modes_config.ess_adv_unbonded_slow_timeout;
                adv_params.interval    = m_adv_modes_config.ess_adv_unbonded_slow_interval;
                adv_params.fp          = BLE_GAP_ADV_FP_ANY;
                adv_params.p_whitelist = NULL;
                m_advdata.flags        = BLE_GAP_ADV_FLAGS_LE_ONLY_GENERAL_DISC_MODE;
                err_code               = ble_advdata_set(&m_advdata, NULL);
            ...
            m_adv_evt = ESS_ADV_EVT_MODE_2;
            break;
            
        case ESS_ADV_MODE_3:
            ADV_LOG("[ADV]: AdvMode=3.\r\n");
            if (whitelist_has_entries(&m_whitelist)) {
                ADV_LOG(" +whitelist.\r\n");
                adv_params.timeout     = m_adv_modes_config.ess_adv_bonded_wl_off_slow_timeout;
                adv_params.interval    = m_adv_modes_config.ess_adv_bonded_wl_off_slow_interval;
                adv_params.fp          = BLE_GAP_ADV_FP_ANY;
                adv_params.p_whitelist = NULL;
                m_advdata.flags        = BLE_GAP_ADV_FLAGS_LE_ONLY_GENERAL_DISC_MODE;
                err_code               = ble_advdata_set(&m_advdata, NULL);
                VERIFY_SUCCESS(err_code);
                m_adv_evt = ESS_ADV_EVT_MODE_3;
            } else {
                ADV_LOG(" -whitelist.\r\n");
                m_adv_mode_current = ESS_ADV_MODE_IDLE;
                m_adv_evt = ESS_ADV_EVT_IDLE;
            }
            break;
            
        case ESS_ADV_MODE_IDLE:
            ADV_LOG("[ADV]: AdvMode=0.\r\n");
            break;
    }
    
//    if (whitelist_has_entries(&m_whitelist)) {
//        ADV_LOG("[ADV]: Use whitelist.\r\n");
//        if (m_adv_mode_current == ESS_ADV_MODE_UNBONDED_FAST) {
//            m_adv_mode_current = ESS_ADV_MODE_BONDED_WL_ON;
//        }
//        switch (m_adv_mode_current) {
//            case ESS_ADV_MODE_BONDED_WL_ON:
//                adv_params.timeout  = m_adv_modes_config.ess_adv_bonded_wl_on_timeout;
//                adv_params.interval = m_adv_modes_config.ess_adv_bonded_wl_on_interval;
//                m_adv_evt = ESS_ADV_EVT_BONDED_WL_ON;
//                ADV_LOG("[ADV]: Starting bonded advertisement with whitelist.\r\n");
//                break;

//            case ESS_ADV_MODE_BONDED_WL_OFF_FAST:
//                adv_params.timeout  = m_adv_modes_config.ess_adv_bonded_wl_off_fast_timeout;
//                adv_params.interval = m_adv_modes_config.ess_adv_bonded_wl_off_fast_interval;
//                m_adv_evt = ESS_ADV_EVT_BONDED_WL_OFF_FAST;
//                ADV_LOG("[ADV]: Starting bonded fast advertisement without whitelist.\r\n");
//                break;

//            case ESS_ADV_MODE_BONDED_WL_OFF_SLOW:
//                adv_params.timeout  = m_adv_modes_config.ess_adv_bonded_wl_off_slow_timeout;
//                adv_params.interval = m_adv_modes_config.ess_adv_bonded_wl_off_slow_interval;
//                m_adv_evt = ESS_ADV_EVT_BONDED_WL_OFF_SLOW;
//                ADV_LOG("[ADV]: Starting bonded slow advertisement without whitelist.\r\n");
//                break;
...
//        }
//    } else {
//        ADV_LOG("[ADV]: Do not use whitelist.\r\n");
//        switch (m_adv_mode_current) {
//            case ESS_ADV_MODE_UNBONDED_FAST:
//                adv_params.timeout  = m_adv_modes_config.ess_adv_unbonded_fast_timeout;
//                adv_params.interval = m_adv_modes_config.ess_adv_unbonded_fast_interval;
//                m_adv_evt = ESS_ADV_EVT_UNBONDED_FAST;
//                ADV_LOG("[ADV]: Starting unbonded fast advertisement.\r\n");
//                break;

//            case ESS_ADV_MODE_UNBONDED_SLOW:
//                adv_params.timeout  = m_adv_modes_config.ess_adv_unbonded_slow_timeout;
//                adv_params.interval = m_adv_modes_config.ess_adv_unbonded_slow_interval;
//                m_adv_evt = ESS_ADV_EVT_UNBONDED_SLOW;
//                ADV_LOG("[ADV]: Starting unbonded slow advertisement.\r\n");
//                break;
...
//    }
    ...
    if (m_adv_mode_current != ESS_ADV_MODE_IDLE)
    {
        ADV_LOG_PRINTF("[ADV]: Interval: %d\r\n", adv_params.interval);
        ADV_LOG_PRINTF("[ADV]: Timeout: %d\r\n", adv_params.timeout);
...
void ess_advertising_on_ble_evt(ble_evt_t const * p_ble_evt)
{
...
//            m_whitelist_temporarily_disabled = false;
            ...
            {
//               err_code = ble_advertising_start(ESS_ADV_MODE_UNBONDED_FAST);
               m_adv_mode_current = ESS_ADV_MODE_1;
               err_code = ess_advertising_start();
...
//                        break;
                    ...
                    case ESS_ADV_MODE_1:
                    {
                        uint32_t err_code;
                        m_adv_evt = ESS_ADV_EVT_MODE_1;
                        ADV_LOG("[ADV]: Timed out from mode 1 advertising, starting mode 2 advertising.\r\n");
                        m_adv_mode_current = ESS_ADV_MODE_2;
                        err_code = ess_advertising_start();
                        if ((err_code != NRF_SUCCESS) && (m_error_handler != NULL))
                        {
                            m_error_handler(err_code);
                    ...
                    case ESS_ADV_MODE_2:
                    {
                        uint32_t err_code;
                        m_adv_evt = ESS_ADV_EVT_MODE_2;
                        ADV_LOG("[ADV]: Timed out from mode 2 advertising, starting mode 3 advertising.\r\n");
                        m_adv_mode_current = ESS_ADV_MODE_3;
                        err_code = ess_advertising_start();
                        if ((err_code != NRF_SUCCESS) && (m_error_handler != NULL))
                        {
                            m_error_handler(err_code);
                    ...
                    case ESS_ADV_MODE_3:
                    {
                        uint32_t err_code;
                        m_adv_evt = ESS_ADV_EVT_MODE_3;
                        ADV_LOG("[ADV]: Timed out from mode 3 advertising, stopping advertising.\r\n");
                        m_adv_mode_current = ESS_ADV_MODE_IDLE;
                        err_code = ess_advertising_start();
                        if ((err_code != NRF_SUCCESS) && (m_error_handler != NULL))
                        {
...
//                        uint32_t err_code;
//                        m_adv_evt = ESS_ADV_EVT_UNBONDED_FAST;
//                        ADV_LOG("[ADV]: Timed out from unbonded fast advertising, starting unbonded slow advertising.\r\n");
//                        m_adv_mode_current = ESS_ADV_MODE_UNBONDED_SLOW;
//                        err_code = ess_advertising_start();
//                        if ((err_code != NRF_SUCCESS) && (m_error_handler != NULL))
//                        {
//                            m_error_handler(err_code);
...
//                    
//                    case ESS_ADV_MODE_UNBONDED_SLOW:
//                        m_adv_evt = ESS_ADV_EVT_UNBONDED_IDLE;
//                        ADV_LOG("[ADV]: Timed out from unbonded slow advertising, stopping advertising.\r\n");
//                        m_adv_mode_current = ESS_ADV_MODE_UNBONDED_FAST;
//                        if (m_evt_handler != NULL)
//                        {
//                            m_evt_handler(m_adv_evt);
//                        }
...
//                        uint32_t err_code;
//                        m_adv_evt = ESS_ADV_EVT_BONDED_WL_ON;
//                        ADV_LOG("[ADV]: Timed out from bonded advertising with whitelist, starting bonded fast advertising without whitelist.\r\n");
//                        m_adv_mode_current = ESS_ADV_MODE_BONDED_WL_OFF_FAST;
//                        err_code = ess_advertising_start();
//                        if ((err_code != NRF_SUCCESS) && (m_error_handler != NULL))
//                        {
//                            m_error_handler(err_code);
...
//                        uint32_t err_code;
//                        m_adv_evt = ESS_ADV_EVT_BONDED_WL_OFF_FAST;
//                        ADV_LOG("[ADV]: Timed out from bonded fast advertising without whitelist, starting bonded slow advertising without whitelist.\r\n");
//                        m_adv_mode_current = ESS_ADV_MODE_BONDED_WL_OFF_SLOW;
//                        err_code = ess_advertising_start();
//                        if ((err_code != NRF_SUCCESS) && (m_error_handler != NULL))
//                        {
//                            m_error_handler(err_code);
...
//                    
//                    case ESS_ADV_MODE_BONDED_WL_OFF_SLOW:
//                        m_adv_evt = ESS_ADV_EVT_BONDED_IDLE;
//                        ADV_LOG("[ADV]: Timed out from bonded slow advertising without whitelist, stopping advertising.\r\n");
//                        m_adv_mode_current = ESS_ADV_MODE_BONDED_WL_ON;
//                        if (m_evt_handler != NULL)
//                        {
//                            m_evt_handler(m_adv_evt);
